...
lidWallHeight = 7;
baseWallHeight = AcdcHeight + 3 + (relayHeight + 3) + 3; //-- Height of the base walls
// shellWidth = main_width + paddingLeft + paddingRight;
...
// inspectX = 3; //-> 0=none (>0 from Back)
// inspectY = 3; //-> 0=none (>0 from Right)
...
[
  ...
  [
    ...
    0,
    1.6, // thickness
    AcdcHeight + 3 + (relayHeight + 3), // standoffHeight
    7, // standoffDiameter
  ...
  [
    ...
    50 + 2 + 1,
    15 + 1,
    main_length / 2 - (relay_length / 2),
    main_width / 2 - (relay_width / 2),
    1.5,
    AcdcHeight + 3,
    5,
    1.7,
  ],
  ["power", 32, 22, 12, 0, AcdcHeight, 0],
];
...
[
  ...
  [1.5, 1.5, default, default, default, default, yappBaseOnly, yappAllCorners, [yappPCBName, "relay"]],
];
...
module baseHookInside() {
  translate([main_length + 7, wallThickness / 2, (basePlaneThickness / 2)]) {
    color("blue") {
      cube([2, (shellWidth - wallThickness * 2 + 2), ( (basePlaneThickness / 2) + baseWallHeight)]);
    }
  }
}
